...
function Drill:set_skill_upgrades(upgrades)
	...
	if self.is_drill or self.is_saw then
		...
		if upgrades.speed_upgrade_level and 2 <= upgrades.speed_upgrade_level then
		...
		elseif upgrades.speed_upgrade_level and 1 <= upgrades.speed_upgrade_level then
		...
		if auto_repair_level > 0 then
			local drill_autorepair_chance = tweak_data.upgrades.values.player.drill_autorepair[auto_repair_level]
