...
function CrimeSpreeRewardsMenuComponent:_texture_loaded_clbk(params, texture_idstring)
	...
	if is_pattern then
		...
		item:set_w(math.round(0.7111111 * panel:h()))
		item:set_h(panel:h())
	end
...
function CrimeSpreeRewardsMenuComponent:_update_rewards_list()
	...
	self._list_scroll = ScrollablePanel:new(self._list_panel, "loot_scroll", {padding = 0, force_scroll_indicators = true})
	local count = 0
	...
	if 0 < self:get_reward("continental_coins") then
