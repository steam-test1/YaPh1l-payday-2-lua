...
function UGCItem:_valid_for_submission()
	...
	if not info.description or 1 > string.len(info.description) then
...
function UGCItem:_load_xml(path, load_node_func, root_node, param_preprocessor)
	...
	for child in node:children() do
		table.insert(to_process, {node = child, parent = root_node})
	end
...
local indent_string = function(str, indent)
	local s = ""
	for i = 1, indent do
		s = s .. "\t"
	end
...
function UGCItem:_save_xml(path, node_name, attributes, params, param_preprocessor)
	...
	if file ~= nil then
		...
		while 1 <= table.getn(to_process) do
			local element = table.remove(to_process)
...
function UGCItem:_load_config()
	local parse_vector = function(key, value)
...
local tags_to_string = function(tags)
	local tag_string
...
local string_to_tags = function(tags_string)
	local tags = {}
