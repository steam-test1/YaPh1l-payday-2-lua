...
function CrewManagementGui:create_mask_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture}, callback(self, self, "open_mask_category_menu", index))
end
function CrewManagementGui:create_weapon_button(panel, index)
	...
	local item = CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, layer = 1} or managers.localization:to_upper_text("menu_crew_defualt"), callback(self, self, "open_weapon_menu", {"primaries", index}))
	if rarity then
...
function CrewManagementGui:create_ability_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, texture_rect = texture_rect}, callback(self, self, "open_ability_menu", index))
end
function CrewManagementGui:create_skill_button(panel, index)
	...
	return CrewManagementGuiLoadoutItem:new(self, panel, {texture = texture, texture_rect = texture_rect}, callback(self, self, "open_skill_menu", {"skill", index}))
end
...
function CrewManagementGuiButton:set_selected(state)
	...
	if self._selected_changed then
		local _ = self:_selected_changed(state)
	end
end
