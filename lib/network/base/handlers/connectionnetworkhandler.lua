...
function ConnectionNetworkHandler:request_stockholm_syndrome(pos, peer_id)
	local peer = managers.network:session():peer(peer_id)
	if not managers.trade:trade_in_progress() then
		managers.player:init_auto_respawn_callback(pos, peer_id, true)
		managers.network:session():send_to_peer(peer, "stockholm_syndrome_results", true)
	else
		managers.network:session():send_to_peer(peer, "stockholm_syndrome_results", false)
	end
end
function ConnectionNetworkHandler:stockholm_syndrome_results(can_trade)
	managers.player:send_message(Message.CanTradeHostage, nil, can_trade)
end
