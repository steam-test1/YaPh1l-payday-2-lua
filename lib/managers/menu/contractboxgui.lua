...
function ContractBoxGui:create_contract_box()
	...
	if contact_data then
		...
		if managers.mutators:are_mutators_enabled() then
			if managers.mutators:allow_mutators_in_level(job_chain and job_chain[1] and job_chain[1].level_id) then
...
function ContractBoxGui:check_update_mutators_tooltip()
	...
	if not managers.mutators:are_mutators_enabled() then
		...
		elseif lobby_data then
			...
			refresh_contract = lobby_data or self._contract_panel:child("mutators_text") ~= nil
		end
	end
