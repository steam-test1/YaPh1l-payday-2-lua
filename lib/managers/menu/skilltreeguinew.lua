...
local make_fine_text = function(text_gui)
...
function NewSkillTreeGui:_setup()
	...
	local tab_panel = skills_panel:panel({name = "TabPanel", h = PAGE_TAB_H})
	tab_panel:set_top(0)
	...
	local pages = {}
	for tree, data in ipairs(skilltrees_tweak) do
		pages[data.skill] = pages[data.skill] or {}
		table.insert(pages[data.skill], tree)
	end
	local page_data
	local tab_x = 0
...
function NewSkillTreeGui:refund_point(item)
	local skill_id = item:skill_id()
	local step = self._skilltree:next_skill_step(skill_id)
	local can_remove = self._skilltree:skill_can_be_removed(item:tree(), item:tier(), skill_id)
	local completed = self._skilltree:skill_completed(skill_id)
	local skill_data = tweak_data.skilltree.skills[skill_id]
	if step == 1 or not can_remove then
		item:flash()
		return
	end
	if item:refund() then
...
function NewSkillTreeTierItem:item(skill_id)
	return not self._skills[skill_id] and self._skills_ordered[1] and self._skills[self._skills_ordered[1]]
end
...
function NewSkillTreeSkillItem:is_active()
	return self._skilltree:next_skill_step(self._skill_id) > 1
end
function NewSkillTreeSkillItem:is_completed()
...
function NewSkillTreeSkillItem:refresh()
	...
	icon:set_alpha(self._selected and 0.7 or 0.5 or step > 1 and 1 or self._selected and 0.8 or 0.6)
...
function NewSkillTreeSkillItem:refund()
	...
	if skill_level > 0 then
		...
		if skill_tree:is_refund_acceptable(skill_id, tier, tree) and skill_tree:refund_skill(tree, skill_id) then
