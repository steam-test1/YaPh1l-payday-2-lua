...
function CommunityChallengesManager:_migrate_old_statistic(old_stat, new_stat, ratio, limit)
	repeat
		...
		if old_stat_value > 0 and Steam:sa_handler():set_stat(old_stat, old_stat_value - chunk) and Steam:sa_handler():set_stat(new_stat, new_stat_value + math.floor(chunk * ratio)) then
