...
function NewHeistsGui:set_bar_width(w, random)
	...
	if random or not self._bar_x then
	end
	self._bar_x = math.random(1, 255)
	if random or not self._bar_y then
	end
	self._bar_y = math.random(0, math.round(self._bar:texture_height() / 2 - 1)) * 2
	local x = self._bar_x
	local y = self._bar_y
	local h = 6
...
function NewHeistsGui:_move_pages(pages)
	...
	local function swipe_func(o, other_object, swipe_distance, time, end_pos)
		time = math.max(1.0E-4, time or 1)
