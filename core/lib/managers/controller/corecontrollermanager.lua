	...
	elseif (SystemInfo:platform() == Idstring("PS4") or SystemInfo:platform() == Idstring("XB1")) and self._default_controller_list then
		...
		for _, controller in ipairs(self._default_controller_list) do
			if not controller:connected() then
				...
				if SystemInfo:platform() == Idstring("XB1") and controller:type() == "xb1_controller" and (controller:device_id() ~= self._controller_device_id or XboxLive:current_user() and controller:user_xuid() ~= XboxLive:current_user()) then
					connected = false
