...
function CustomSafehouseManager:unlocked()
	return Global.mission_manager.has_played_tutorial and (managers.experience:current_level() >= tweak_data.safehouse.level_limit or managers.experience:current_rank() > 0)
end
...
function CustomSafehouseManager:can_afford_any_upgrade()
	...
	for room_id, data in pairs(self._global.rooms) do
		...
		if tier and prices[tier] ~= nil and cheapest_upgrade > prices[tier] then
...
function CustomSafehouseManager:interval_til_new_daily()
	if Global.custom_safehouse_manager.daily.state ~= "rewarded" or not 16 then
	end
