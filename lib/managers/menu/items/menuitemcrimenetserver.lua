...
function MenuItemCrimeNetServer:setup_gui(node, row_item)
	...
	for i = start_difficulty, num_difficulties do
		local skull = difficulty_panel:bitmap({
			...
			alpha = i > desired_difficulty and 0.5 or 1,
			blend_mode = i > desired_difficulty and "normal" or "add",
			layer = 0,
			color = i > desired_difficulty and Color.black or tweak_data.screen_colors.risk
		})
		x = x + 11
	end
