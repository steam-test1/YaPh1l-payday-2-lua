...
function SkillTreeManager:skill_can_be_removed(tree, tier, skill_id)
	...
	local points_left = self:get_points_spent_until_tier(tiers, tier) - self:_points_spent_skill(tier, skill_id)
	for tier_index, tier_data in pairs(tiers) do
...
function SkillTreeManager:is_refund_acceptable(skill_id, current_tier, tree)
	local skilltree_tweak = tweak_data.skilltree
	local tiers = skilltree_tweak:get_tiers(tree)
	if current_tier >= #tiers then
		return true
	end
	local next_tier, num_unlocked_skills_above = self:_get_heighst_tier_points(current_tier, tiers)
	if num_unlocked_skills_above <= 0 then
		return true
	else
		local tier_cost = tweak_data.skilltree:get_tier_unlocks()[next_tier]
		local skill_cost = self:skill_cost(current_tier, self:skill_step(skill_id))
		local points = self:get_points_spent_until_tier(tiers, current_tier) - skill_cost
		if tier_cost <= points then
			return true
		end
	end
	return false
...
function SkillTreeManager:refund_skill(tree, skill_id)
	local skills = self._global.skills
