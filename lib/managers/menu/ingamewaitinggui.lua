...
function IngameWaitingGui:set_panel_for(peer_id)
	...
	self._loadout_panel = self._content_panel:panel()
	local primary_panel = self._loadout_panel:panel({w = 128, h = 128})
	local secondary_panel = self._loadout_panel:panel({w = 128, h = 128})
	...
	local melee_panel = self._loadout_panel:panel({w = 128, h = 64})
	local throw_panel = self._loadout_panel:panel({w = 128, h = 64})
	local deploy_panel = self._loadout_panel:panel({w = 64, h = 64})
	local armor_panel = self._loadout_panel:panel({w = 64, h = 64})
...
function IngameWaitingGui:mouse_pressed(button, x, y)
	...
	if self._prev_arrow:panel():inside(x, y) then
		local t = self:prev_page()
	end
end
function IngameWaitingGui:set_current_button(index)
	...
	self._current_btn = {button = target, index = index}
