...
LootDropManager.new_make_mass_drop = function(self, amount, item_pc, return_data, setup_data)
	local plvl = managers.experience:current_level()
	local pstars = managers.experience:level_to_stars()
	local stars = pstars
	local pc = stars * 10
	...
	if setup_data then
		local droppable_items, maxed_inventory_items = self:droppable_items(item_pc or 40, infamous_success, setup_data.skip_types)
	end
	local global_value, entry = nil, nil
	return_data.items = {}
	for i = 1, amount do
		...
		if not setup_data or not setup_data.preferred_type_drop then
			local pc_type = self:_get_type_items(normalized_chance, true)
		end
		if not droppable_items[pc_type] then
			local drop_table = maxed_inventory_items[pc_type]
		end
		...
		local item = {global_value = global_value, type_items = pc_type, item_entry = entry}
