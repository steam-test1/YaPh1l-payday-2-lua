...
function CoreEnvironmentControllerManager:refresh_render_settings(vp)
	...
	color_grading = self._ignore_user_color_grading or managers.user:get_setting("video_color_grading") or self._default_color_grading
...
function CoreEnvironmentControllerManager:set_ao_setting(setting, vp)
	...
	if vp then
		...
		if setting == "off" or not Idstring("AO_mask_weapon") then
		end
		vp:set_post_processor_effect("World", ids_ao_mask_post_processor, (Idstring("AO_off")))
